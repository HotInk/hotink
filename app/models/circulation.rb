class Circulation < ActionMailer::Base
  ...
  default_url_options[:host] = "setup.hotink.net"  
  ...
  def user_invitation(account, invite)
    ...
    sent_on       Time.now  
    body          :edit_invitation_url => edit_user_invitation_url(invite, :subdomain => account.name), :admin_url => admin_url(:subdomain => account.name)  
  end
